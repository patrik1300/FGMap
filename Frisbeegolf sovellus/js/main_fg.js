...
function lisaaMarker(crd, radat, teksti) {
    ...
    merkki
        ...
        try {
            ...
            document.getElementById(
                'arvostelu').innerHTML = 'pisteet:' + '</br>' + radat.rating;

            document.getElementById('arvostelu').src = radat.rating_img;
...
function haeFGradat(crd) {
    poistaMarkerit()
    merkit.addTo(map)
    fetch(`https://cors-anywhere.herokuapp.com/https://www.dgcoursereview.com/api/index.php?key=6h2cozd1c70cy2dzuzas00im&mode=near_rad&lat=${crd.latitude}&lon=${crd.longitude}&limit=5&rad=15&sig=bcc2f23f9b870904df1e697f15e0b849`).
    then(function(vastaus) {
        console.log('palvelimen vastaus', vastaus);
        return vastaus.json();
    }).
